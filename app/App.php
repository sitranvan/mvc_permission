...
class App
{
    ...
    public function handleUrl()
    {
        ...
        $this->handlePermission($this->getUrlOriginalRoute());
        ...
        if (empty($urlCheck)) {
            ...
            $urlCheck =  'clients/posts/' . $this->controller;
        }
        // Xử lí controller
    ...
    public function getUrlOriginalRoute()
    {
        return $this->routes->getOriginallUrl();
    }
    ...
    public function handlePermission($urlOriginal)
    {
        global $permissionMiddleware;
        $file  =  'app/middlewares/' . $permissionMiddleware . '.php';
        if (file_exists($file)) {
            if (class_exists($permissionMiddleware)) {
                $permission = new $permissionMiddleware();
                $permission->handle($urlOriginal);
            }
        }
    }
